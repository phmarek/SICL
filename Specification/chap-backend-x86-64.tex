...
% Stack pictures: please show address direction (higher/lower addresses), and 
% stack grow direction. I'd have expected RSP to be at the lower bottom, BTW 
% - it is the lowest address on x86 after all...
...
\item Compute the callee function object and the arguments into
  temporary locations.
\item Store the arguments in RDI, RSI, RDX, RCX, and R8.
  % what about optional arguments? Special value (unbound)?
\item Store the argument count in R9 as a fixnum.
  % Even for a fixed number of arguments and some optimization level?
\item Load the static environment of the callee from the callee
  function object into R10.
...
% Are these LAMBDA, LABELS, and FLET only? Because normal calls shouldn't inline 
% to facilitate redefinition.
...
\item Store the values to return in RAX, RDX, RCX, RSI.
  % Do that later to have more registers available for computing the other 
  % values?
...
\begin{itemize}
  ...
  \item An entry representing a \texttt{block} or a \texttt{tagbody}.
    ...
    % Why a timestamp? why not the pointer to the symbol? Might help debugging 
    % as well?!
...
% show the variable in the picture below?
...
% this should happen in lockstep:
% The cleanup forms in 
%    (LET ((*foo* 1))
%      (unwind-protect
%         ...
%         ))
% should still see the binding, right?
...
We obtain this sequence by pushing the keyword arguments to the stack
