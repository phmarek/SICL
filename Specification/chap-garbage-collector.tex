...
% if they "never" move, can that mean OOM for a thread?
...
% so the number of dyads becomes configurable?
...
64-bit word.
% 64bit split done even on 32bit platforms?
...
% 16MB RAM is a lot on a 256MB Rpi...
...
These symbols are exported from the package
...
% What is "too big"? CL:MAKE-ARRAY should already know, how would
% programmers find out?
...
% Not reclaimed by GC at some time? How would I free it if I (resp. my foreign 
% function) detects an error before the header gets allocated?
...
break table is built for racks.
% Addresses are non-overlapping, so having a single table would work as well
In the worst case, each of these
...
% Aren't class definitions put in global heap by default? Do they need promotion 
...
% This list is the same as above, but better structured.
