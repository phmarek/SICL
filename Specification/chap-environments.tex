...
\item etc.
  % Can rigid classes have multiple versions as well?
\end{itemize}
...
\item A mapping from function names (symbols and lists like \texttt{(SETF 
  function)}) to entries representing functions,
...
% environment passed in as NIL below means current environment?

% change all environment argument names below to "env" to reduce the overfull 
% hboxes?

% change new-def to "new" for more consistency?
...
If \textit{function-name} is defined as a macro, then a list 
...
\Defgeneric {(setf compiler-macro-function)} {new-def function-name environment}
...
\texttt{t}.
% Even if the compiler can deduce a type itself?
...
% so, when called for random names, this fills up memory?
...
% can new-class be (CLASS-OF some-instance), ie a standard-class?
...
\Defgeneric {find-package} {name from-package environment}
...
% What about package-local nicknames? Needs a "from" package then as well?!
...
\Defgeneric {package-nicknames} {package from-package environment}
...
\Defgeneric {(setf package-nicknames)} {new-names package from-package environment}

Associate the strings in the list \textit{new-names} as nicknames of
\textit{package} in \textit{environment}.

% Is there a package (write)protection per environment?
...
counter%
% Really each PC? Or only when the source line/stack layout etc. changes?
