...
\item \texttt{011}.  This tag is used for various \emph{immediate}
  ...
  \refsec{sec-data-representation-immediate-objects}).
...
\item \texttt{111}.  This tag is used to tag a pointer to a
  ...
  argument.
  % Perhaps use -1 instead of +7, in case that offset is better supported
  % by cpu instructions? Would amount to mostly the same representation.
\end{itemize}

On a 64-bit machine, floats of type \texttt{short-float} and
\texttt{single-float} are represented as immediate values.

% So long-floats (== double?) are always objects?
...
% "small" just means that there's an alignment hole in every instance.
% Have a pointer to the corresponding class definition instead?
...
with the array. 
% Concurrent updates in multiple threads?
...
% Shouldn't GF depend on (and USE) the exact class definition as well?

% What about concurrent class redefinitions and/or their use?
...
% why not a vector? might be faster?!
...
% So, is it then flexible (standard-object) or rigid (function)?
...
\item Three tables providing support for the \emph{step} commands of
  ...
  tables.
% Perhaps indent the following tables as sub-points of the one above?
...
\item A table mapping a value of the program counter to a
  ...
  See also the \reffig{fig-root-finding-example-1} in 
  \refchap{chap-garbage-collector}.
...
\begin{figure}
\begin{center}
  \includegraphics[width=4cm]{fig-repr-array.pdf}
\end{center}
...
% Well, a # strings -n 20 <sbcl-core-image>  | wc
% tells me that there's 760kB of ASCII-strings embedded; in a 32bit
% representation, that would be 2MB more RAM and disk space wasted.
% Furthermore, when writing strings to disk or the network (hunchentoot etc.!) 
% a BASE-STRING can just be passed on to the OS - 32bits must be converted
% byte-for-byte to UTF8 first, requiring some CPU...
Thus the rack of a
...
\item The \emph{entry point}.  The entry
  point is a \emph{raw address} of an aligned word 
    % but code is just an array of bytes -- will there only be one entry point? 
  % perhaps another one for optimized calls with keyword arguments translated or 
  % so?
  in the vector
  ...
  % dump-image makes no GC compaction?
\end{enumerate}
