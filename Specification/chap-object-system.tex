...
% shouldn't AMOP (as newer document) take precedence?
...
\footnote{It might be possible for the garbage collector to change the
  ...
  % Well, then the pointer to the slots wouldn't be needed - the old stamp can 
  % just be used for lookup.
...
% definition*s*, in case there were multiple modifications? Done step-by-step?
...
% see above, and the idea of keeping old class definitions while they're needed
...
% Combine both paragraphs above? Very similar content.
...
% A worst-case szenario: Have a parent class P with two child classes C0 and C1, 
% then change C1's definition multiple times (gets consecutive numbers), and 
% call a generic function only defined for P. Would that be caught?
